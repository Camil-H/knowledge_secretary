...
Every framework knob lives in `src/config.py`, grouped by area: the shared HTTP timeout and retry/backoff bounds, the Gemini model table (`GEMINI_TEXT_MODELS` — each row's limits, and its `search` flag, which decides whether the podcast's research may draw on it), the OpenRouter ranking (`OPENROUTER_PREFERRED_CONTEXT`), episode length (`TRANSCRIPT_PARTS`, `TRANSCRIPT_PART_BUDGETS`), the TTS voices and character budget, and the single `RETENTION_DAYS` window that governs how much history the site renders, how long a podcast release is hosted and how long an id stays "seen". Three of those knobs read an env var when one is set — `SITE_TITLE`, `SITE_SUBTITLE` and `LLM_RATE_LIMIT_RETRIES` — so they can be changed without editing code; everything else is edited in place. What is deliberately *not* there stays next to the code that owns it: the prompt files, the `<Person1>` turn pattern, and the wire literals each provider dictates. Per-task sources live in each task's `sources.yaml` (below). The only runtime inputs are secrets, set as GitHub Actions repository secrets:
...
**4. Branding & editorial voice.** Site title/subtitle: the `SITE_TITLE`/`SITE_SUBTITLE` env vars, whose defaults are the constants of the same name in `src/config.py`. Per-task subject lines: `src/tasks/*/task.py`. Podcast host roles and voices: `src/tasks/podcast/transcript_prompt.md` and `TTS_VOICES` in `src/config.py`. The editorial framing (currently biotech/pharma) lives in the prompts — `src/tasks/newsletter/prompt.md`, `src/tasks/youtube/prompt.md`, `src/tasks/podcast/transcript_prompt.md`, and `src/tasks/podcast/research_prompt.md`. Update the `LICENSE` copyright line too.
