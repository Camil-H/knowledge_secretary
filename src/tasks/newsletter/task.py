...
def _produce(ctx: Context, items: list[Item]) -> str:
    ...
    is dropped."""
    if config.NEWSLETTER_ITEM_CHAR_FLOOR * len(items) > config.NEWSLETTER_TOTAL_CHAR_BUDGET:
...
def _map_reduce(ctx: Context, items: list[Item]) -> str:
    ...
    batch_size = config.NEWSLETTER_TOTAL_CHAR_BUDGET // config.NEWSLETTER_ITEM_CHAR_FLOOR
    fragments = [
        ctx.call(
            system=EDITOR_PROMPT, user=_editor_input(list(batch), config.NEWSLETTER_ITEM_CHAR_FLOOR)
        )
...
def _per_item_budget(n_items: int) -> int:
    """Per-item body budget: an equal share of the total char budget, clamped to the per-item
    floor and limit."""
    return max(
        config.NEWSLETTER_ITEM_CHAR_FLOOR,
        min(config.NEWSLETTER_TOTAL_CHAR_BUDGET // n_items, config.NEWSLETTER_ITEM_CHAR_LIMIT),
    )
