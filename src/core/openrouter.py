...
# models(): a preferred id absent from the current live list is simply skipped.
PREFERRED_CONTEXT = [
...
def call(system: str, user: str, max_tokens: int | None) -> str:
    ...
    every model x retry."""
    candidates = models() or [FALLBACK_MODEL]
...
def _free_models(*, limit: int = _FREE_LIMIT) -> list[str]:
    ...
    try:
        data: list[ModelRecord] = httpx.get(MODELS_URL, timeout=_HTTP_TIMEOUT_S).json()["data"]
